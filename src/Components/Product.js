...
export default function Product({name, index, shoppingLists, setShoppingLists, useRefresh}) {
  ...
  const edit = async () => {
    ...
    const containList = shoppingLists.some(shoppingList => shoppingList === newEntry);
    ...
    newShoppingLists[index] = newEntry;
    ...
    <>
      {editable ? (
        ...
        <View style={styles.listNameView}>
          <Text style={styles.listName}>{newEntry}</Text>
